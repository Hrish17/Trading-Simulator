    ...
    for (int i = n; i < dates1.size(); i++)
    {
        ...
        if (zScore > threshold)
        {
            if (NumOfStocks < x)
            {
                ...
                OrderFile1 << dates1[i]->day << "/" << dates1[i]->month << "/" << dates1[i]->year << ",SELL,1," << CurrCash1 << endl;
                OrderFile2 << dates2[i]->day << "/" << dates2[i]->month << "/" << dates2[i]->year << ",BUY,1," << CurrCash2 << endl;
                Cash += spread;
            }
        }
        else if (zScore < -threshold)
        {
            if (NumOfStocks > -x)
            {
                ...
                string CurrCash1 = formatDouble(dates1[i]->cprice);
                string CurrCash2 = formatDouble(dates2[i]->cprice);
                OrderFile1 << dates1[i]->day << "/" << dates1[i]->month << "/" << dates1[i]->year << ",BUY,1," << CurrCash1 << endl;
                OrderFile2 << dates2[i]->day << "/" << dates2[i]->month << "/" << dates2[i]->year << ",SELL,1," << CurrCash2 << endl;
                Cash -= spread;
            }
        }
        string CurrCash = formatDouble(Cash);
        CashFile << dates1[i]->day << "/" << dates1[i]->month << "/" << dates1[i]->year << "," << fixed << setprecision(2) << CurrCash << endl;
    }
